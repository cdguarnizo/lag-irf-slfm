...
function [M,P,D] = rts_smooth(M,P,A,Q,B,u)
  ...
  % Do the same for B and u if there are any inputs
  if nargin > 4
     if size(B,3) == 1
        B = repmat(B,[1 1 size(M,2)]);
     end
     if size(u,2) == 1
        u = repmat(u,[1 size(M,2)]);
     end
  end
  ...
  for k=(size(M,2)-1):-1:1
    ...
    D(:,:,k) = P(:,:,k) * A(:,:,k)' / P_pred;
    if nargin > 4
        M(:,k)   = M(:,k) + D(:,:,k)*(M(:,k+1) - A(:,:,k)*M(:,k) - B(:,:,k)*u(:,k));
    else
